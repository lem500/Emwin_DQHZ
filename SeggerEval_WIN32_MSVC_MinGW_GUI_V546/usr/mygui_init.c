...
void WinKeyProcess(void)
{
    unsigned char winindex = 0;
	 ...
	 case GUI_KEY_DOWN://下键
          break;
	 case GUI_KEY_ESCAPE://取消键          if((WinIndex !=WINDOW_W27_PASSWORDWIN)&&((WinLastIndex !=WINDOW_W28_INPUT_TEXT)))          {
		   winindex = WinClass[WinIndex].QuitWin;
			if(winindex == 0)
			{
			  break;
			}
			WinLoad(winindex);          }          if(WinLastIndex == WINDOW_W28_INPUT_TEXT)            {                  WinLastIndex = 0;            }          WinKeyValue = 0;
            break;
